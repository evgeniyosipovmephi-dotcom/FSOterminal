...
## Ограничения железа STM32 (важно для тестов и подбора окна)
- **TX-буфер STM32 (FSO-сторона): 512 байт**
- Канал FSO: 24000 бод → **2400 байт/с** реальный максимум (8N1: 10 бит/байт)
- Буфер дренируется за ~0.2 сек при простое
- WINDOW=4 даёт burst 4×255=1020 байт > 512 — буфер может переполниться
- Безопасный burst без оверфлоу: WINDOW≤2 (2×255=510 байт)
- При оверфлоу STM32 перестаёт принимать байты → PROBE RTT резко растёт пока буфер не опустеет
- Исходно рассматривался буфер 64 КБ — при нём дренаж занял бы ~26 сек, что неприемлемо для интерактивности
- **Диагностика оверфлоу:** измерять PROBE RTT во время передачи файла — если RTT растёт >2× от базового, буфер переполняется

## Результаты тестирования производительности (28.05.2026)
| Параметр | Значение |
|---|---|
| Канал | FSO 24000 бод (провода), COM16 ↔ COM13 |
| Оптимальный WINDOW | **1** |
| Скорость (10 KB) | **~1530 байт/с (64% от 2400)** |
| Ретрансмиты | **0** |

**Почему WINDOW=1:**
- WINDOW=2 → burst 510 байт + ретрансмит 510 байт = 1020 байт > 512 буфер → каскад дропов → 46 ретрансмитов
- WINDOW=1 → burst 255 байт, ретрансмит 255 байт — всегда влезает в буфер

**Почему 64%, а не 96% (теория для WINDOW=1):**
- STM32 DMA-таймер опрашивает RX каждые 5 мс → задержка на пути ACK
- Реальный RTT ≈ 157 мс вместо теоретических 108 мс
- Это ограничение прошивки STM32 (чёрный ящик, не меняется)
- `TIMEOUT_NONBLOCKING` в jSerialComm не помог — задержка не на стороне Java
