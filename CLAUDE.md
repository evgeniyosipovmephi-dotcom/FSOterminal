...
### Архитектура передачи (FSO-сторона)
- **Входной буфер:** 512 байт (данные от USB/COM)
- **Выходной буфер:** 512 байт (данные для отправки по FSO)
- **Цикл батчинга:** каждые N мс всё содержимое входного буфера пакуется в PtPP2+DCmd и отправляется в выходной буфер
- **Лимит на пакет: ≤ 255 байт** — если накопилось больше, пакет не упаковывается
- **USB CDC работает на полной скорости USB** (не ограничен 115200 бод!) — весь Java-кадр прибывает в STM32 за < 1 мс

### Почему WINDOW должен быть = 1
- USB доставляет данные мгновенно → при WINDOW=2 оба кадра (2×245=490 байт) попадают в буфер одновременно
- 490 байт > 255 → пакет не упаковывается → данные теряются → каскад ретрансмитов
- WINDOW=1: один кадр = 245 байт < 255 → всегда упаковывается ✓

### Формат упаковки на FSO-канале (PtPP2 + DCmd)
```
[0x7D][PtPP2_seq 1B][DCmd_group 1B][DCmd_cmd 1B][DCmd_len 1B][наш кадр][CRC32 4B][0x7D]
```
- Накладные расходы: **10 байт** на кадр
- SOF=0xAA не стаффируется PtPP2 (в отличие от старого 0x7E) → экономия 1 байт/кадр
- Итоговый FSO-кадр для MAX_PAYLOAD=240: 245 + 10 = **255 байт**

### Диагностика проблем
- **PROBE RTT растёт > 2× во время передачи** → переполнение буфера STM32
- **Ретрансмиты > 0 на проводном канале** → WINDOW > 1 или payload > 250 байт

## Результаты тестирования производительности

### Frame sweep тест (28.05.2026) — подбор оптимального payload
**Условия:** FSO 24000 бод, COM16 ↔ COM13, WINDOW=1, SOF=0xAA, DMA polling ~2 мс, 3 KB на каждый размер

| Payload, байт | Кадров | Время, с | байт/с | % от теор. | Ретрансм. |
|---|---|---|---|---|---|
| 20 | 150 | 3.78 | 793 | 33.1% | 0 |
| 40 | 75 | 2.54 | 1181 | 49.2% | 0 |
| 60 | 50 | 2.32 | 1293 | 53.9% | 0 |
| 80 | 38 | 2.07 | 1449 | 60.4% | 0 |
| 100 | 30 | 1.94 | 1549 | 64.5% | 0 |
| 120 | 25 | 1.83 | 1640 | 68.3% | 0 |
| 150 | 20 | 1.79 | 1672 | 69.7% | 0 |
| 180 | 17 | 1.71 | 1757 | 73.2% | 0 |
| 210 | 15 | 1.71 | 1752 | 73.0% | 0 |
| **240** | **13** | **1.68** | **1790** | **74.6%** | **0** |

**Оптимум: MAX_PAYLOAD = 240 байт** (кадр = 245 байт, запас 10 байт до лимита 255)

**Почему кривая выполаживается:**
- 20→150 байт: прирост +111% (огромный)
- 150→240 байт: прирост +7% (почти ничего)
- Кривая монотонна — никакого sweet spot при меньших размерах

**Куда уходят оставшиеся 25.4%:**
- PtPP2+DCmd overhead (10 байт/кадр): 240/255 = 94.1% КПД по каналу
- Фиксированная задержка на 4 цикла DMA-полинга + ACK по FSO ≈ 23 мс/RTT

### Сравнение с предыдущим результатом
| Конфигурация | байт/с | % теор. |
|---|---|---|
| WINDOW=1, SOF=0x7E, DMA=20 мс (было) | 1530 | 64% |
| WINDOW=1, SOF=0xAA, DMA=2 мс (сейчас) | 1790 | 74.6% |
| Выигрыш от оптимизаций | **+260** | **+10.6 п.п.** |
